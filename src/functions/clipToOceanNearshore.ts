...
export async function clipOutsideNearshore(
  feature: Feature<Polygon | MultiPolygon>
) {
  if (!isInternalVectorDatasource(ds))
    throw new Error("Expected vector datasource for nearshore");
 ...
 */
export async function clipToNearshore(feature: Feature): Promise<Feature> {
  ...
  let clipped = await clipOutsideNearshore(feature);

  if (!clipped || area(clipped) === 0) {
    throw new ValidationError("Sketch is outside of nearshore boundary");
  } else {
